...
static bool ggml_vk_roll_bk_loop(const uint32_t * code, size_t word_count, std::vector<uint32_t> & out) {
	...
	for (size_t pos = 5; pos < word_count; ) {
		...
		if (op == spv::OpLoopMerge && wc >= 4) {
			loops.push_back({ pos, 0, code[pos + 3] });
		}

		if (op == spv::OpLabel && wc >= 2) {
			for (auto & l : loops) {
				if (l.end == 0 && code[l.header + 1] == code[pos + 1]) {
					l.end = pos;
				}
			}
		}
...
static bool ggml_vk_matmul_int_shmem_support(const vk_device& device, const std::vector<uint32_t>& warptile, bool mul_mat_id, ggml_type src0_type) {
	...
	switch (src0_type) {
		case GGML_TYPE_Q2_0:
			block_a_size = std430_size({{32, 4}, {fp_size,  fp_align}});
			break; // qs[8] + dm
		case GGML_TYPE_Q4_0:
			block_a_size = std430_size({{16, 4}, {fp_size,  fp_align}});
			break; // qs[16/4] + dm
		case GGML_TYPE_Q4_1:
			block_a_size = std430_size({{16, 4}, {fp2_size, fp2_align}});
			break; // qs[16/4] + dm(vec2)
		case GGML_TYPE_Q5_0:
			block_a_size = std430_size({{16, 4}, {4, 4}, {fp_size,  fp_align}});
			break; // qs[16/4] + qh + dm
		case GGML_TYPE_Q5_1:
			block_a_size = std430_size({{16, 4}, {4, 4}, {fp2_size, fp2_align}});
			break; // qs[16/4] + qh + dm(vec2)
		case GGML_TYPE_Q8_0:
			block_a_size = std430_size({{32, 4}, {fp_size,  fp_align}});
			break; // qs[8] + dm
		case GGML_TYPE_MXFP4:
			...
			break; // qs[8] + d
		case GGML_TYPE_Q2_K:
			block_a_size = std430_size({{ 8, 4}, {2, 2}, {fp2_size, fp2_align}});
			break; // qs[2] + scales(u8vec2) + dm(vec2)
		case GGML_TYPE_Q3_K:
			block_a_size = std430_size({{16, 4}, {fp2_size, fp2_align}});
			break; // qs[4] + d_scales(vec2)
		case GGML_TYPE_Q4_K:
			block_a_size = std430_size({{16, 4}, {fp2_size, fp2_align}});
			break; // qs[4] + dm(vec2)
		case GGML_TYPE_Q5_K:
			block_a_size = std430_size({{32, 4}, {fp2_size, fp2_align}});
			break; // qs[8] + dm(vec2)
		case GGML_TYPE_Q6_K:
			block_a_size = std430_size({{32, 4}, {fp2_size, fp2_align}});
			break; // qs[8] + d_scales(vec2)
		default:
			return false;
	}
...
static void ggml_vk_load_shaders(vk_device& device, vk_pipeline requested) {
	...
	for (auto &fa : device->pipeline_flash_attn_f32_f16) {
		...
		if (bf16_kv) {
			...
			if (device->fp16) {
				if (f32acc) {
					spv_data = flash_attn_f32_f16_int8_data;
					spv_size = flash_attn_f32_f16_int8_len;
				} else        {
					spv_data = flash_attn_f32_f16_f16acc_int8_data;
					spv_size = flash_attn_f32_f16_f16acc_int8_len;
				}
			} else {
			...
			if (device->fp16) {
				if (device->dot2_f16) {
					if (f32acc) {
						spv_data = flash_attn_f32_f16_dot2_data;
						spv_size = flash_attn_f32_f16_dot2_len;
					} else        {
						spv_data = flash_attn_f32_f16_dot2_f16acc_data;
						spv_size = flash_attn_f32_f16_dot2_f16acc_len;
					}
				} else {
					if (f32acc) {
						spv_data = flash_attn_f32_f16_data;
						spv_size = flash_attn_f32_f16_len;
					} else        {
						spv_data = flash_attn_f32_f16_f16acc_data;
						spv_size = flash_attn_f32_f16_f16acc_len;
					}
				}
			} else {
				spv_data = flash_attn_f32_f16_fp32_data;
	...
	if (device->coopmat1_fa_support) {
		for (auto &fa : device->pipeline_flash_attn_f32_f16) {
			...
			if (bf16_kv) {
				...
				if (f32acc) {
					spv_data = flash_attn_f32_f16_cm1_data;
					spv_size = flash_attn_f32_f16_cm1_len;
				} else        {
					spv_data = flash_attn_f32_f16_f16acc_cm1_data;
					spv_size = flash_attn_f32_f16_f16acc_cm1_len;
				}
	...
	if (device->coopmat2) {
		for (auto &fa : device->pipeline_flash_attn_f32_f16) {
			...
			if (bf16_kv) {
				...
				if (f32acc) {
					spv_data = flash_attn_f32_f16_cm2_data;
					spv_size = flash_attn_f32_f16_cm2_len;
					name = "flash_attn_f32_f16_aligned_f32acc_cm2";
				} else        {
					spv_data = flash_attn_f32_f16_f16acc_cm2_data;
					spv_size = flash_attn_f32_f16_f16acc_cm2_len;
				...
				if (f32acc) {
					spv_data = flash_attn_f32_f16_cm2_data;
					spv_size = flash_attn_f32_f16_cm2_len;
					name = "flash_attn_f32_f16_f32acc_cm2";
				} else        {
					spv_data = flash_attn_f32_f16_f16acc_cm2_data;
					spv_size = flash_attn_f32_f16_f16acc_cm2_len;
					name = "flash_attn_f32_f16_f16acc_cm2";
				}
			}
			ggml_vk_create_pipeline(device, fa.second, name, spv_size, spv_data, "main", 7,
			                        sizeof(vk_flash_attn_push_constants), {Br, 1, 1},
...
#undef CREATE_MM
	...
	auto const &rm_iq_int = [](uint32_t i) {
		return i == 0 ? 8u : 4u;
	};
		...
		if (conv2d_use_cm1) {
			...
			switch (s) {
				case CONV_SHAPE_64x32:
					conv2d_WM = sg64 ? 32 : 16;
					conv2d_WN = 16;
					break;
				case CONV_SHAPE_64x128:
					conv2d_WM = 32;
					conv2d_WN = sg64 ? 64 : 32;
					break;
				case CONV_SHAPE_32x256:
					conv2d_WM = sg64 ? 16 : 32;
					conv2d_WN = sg64 ? 128 : 32;
					break;
				default:
					break;
			}
	...
	switch (op) {
		...
		case GGML_OP_COL2IM_1D:
			switch (src0->type) {
				...
				case GGML_TYPE_BF16:
					return ctx->device->pipeline_col2im_1d_bf16;
				default:
					return nullptr;
			}
		...
		case GGML_OP_SSM_CONV:
			if (src0->type == GGML_TYPE_F32 && dst->type == GGML_TYPE_F32) {
				switch (ctx->num_additional_fused_ops) {
					...
					case 1:
						return ctx->device->pipeline_ssm_conv_silu_f32;
					case 2:
						return ctx->device->pipeline_ssm_conv_bias_silu_f32;
					default:
						return nullptr;
				}
			}
...
static void ggml_vk_snake_dispatch_fused(ggml_backend_vk_context * ctx, vk_context& subctx, ggml_cgraph * cgraph, int node_idx) {
	...
	switch (x->type) {
		case GGML_TYPE_F32:
			pipeline = ctx->device->pipeline_snake_f32;
			break;
		case GGML_TYPE_F16:
			pipeline = ctx->device->pipeline_snake_f16;
			break;
		case GGML_TYPE_BF16:
			pipeline = ctx->device->pipeline_snake_bf16;
			break;
		default:
